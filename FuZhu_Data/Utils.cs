...
namespace FuZhu_Data
{
    public class Utils
    {
        ...
        /// <returns></returns>
        public static string GetRamCode(DateTime? _DT = null)
        {
            string RamCode;
            #region


            DateTime currentDate = _DT ?? DateTime.Now;
            RamCode = currentDate.ToString("yyyyMMddHHmmss") + Number(4);


            //if (_DT == null)
            //{
            //    RamCode = DateTime.Now.ToString("yyyyMMddHHmmssffff");

            //}
            //else
            //{
            //    RamCode = _DT.ToString("yyyyMMddHHmmssffff");
            //}
        ...
        /// 获得当前绝对路径
        /// </summary>
        /// <param name="strPath">指定的路径:如果\\upload\\downFile</param>
        ...
        public static string GetMapPath(string strPath)
        {
            ...
            strPath = strPath.Replace("/", "\\");
            return Directory.GetCurrentDirectory() + strPath;
        ...
        /// <returns></returns>
        public static void FolderExists(string _folderPath)
        {
            string fullpath = GetMapPath(_folderPath);
        ...
        /// isRewrite
        /// </summary>
        /// <param name="sourcePath">文件原路径</param>
        /// <param name="targetPath">新文件路径</param>
        /// <param name="isRewrite">是否覆盖true=覆盖已存在的同名文件,false则反之</param>
        public static bool FileCopy(string sourcePath, string targetPath, bool isRewrite)
        {
            bool isSuccess = true;
            try
            {
                string sourceFullPath, targetFullPath;
                sourceFullPath = GetMapPath(sourcePath);
                targetFullPath = GetMapPath(targetPath);
                File.Copy(sourceFullPath, targetFullPath, isRewrite);
            }
            catch
            {
                isSuccess = false;
        ...
        public static string OutPageList(int pageSize, int pageIndex, int totalCount, string linkUrl, int centSize)
        {
            ...
            string totalcount = "共<span class=\"current\">" + totalCount + "</span>条&nbsp;";
            ...
            return totalcount + pageStr.ToString();
        }
        #endregion
